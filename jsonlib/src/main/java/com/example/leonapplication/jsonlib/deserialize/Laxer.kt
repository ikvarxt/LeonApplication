...
class Lexer(reader: Reader) {
  ...
  private val tokenMap = hashMapOf<Char, (Char) -> Token>(
    ...
    ']' to { _ -> Token.RBRACKET },
    ':' to { _ -> Token.COLON },
    't' to { _ -> charReader.expectText("rue", valueEndChars); Token.TRUE },
    'f' to { _ -> charReader.expectText("alse", valueEndChars); Token.FALSE },
    'n' to { _ -> charReader.expectText("ull", valueEndChars); Token.NullValue },
    '"' to { _ -> readStringToken() },
